...
> **IMPLEMENTATION NOTE (added Day 5, reconciling this canonical doc with what was actually built — DAILY_BUILD_PLAN.md Day 5 Task 5):** the reference implementation in this repo does **not** route frontend requests through the API as this section specifies. `frontend/dashboard.py` and `frontend/case_detail.py` read the same precomputed parquet artifacts directly via pandas and call `backend/exposure.py`, `backend/audit.py`, and `policy/engine.py` as plain Python functions, not over HTTP — a pragmatic choice made to avoid running and coordinating a second server process during a fast build. This does **not** reintroduce the drift risk this section is written to prevent: all actual computation still lives in exactly one place (`backend/exposure.py`, `graph/scoring.py`) regardless of which code path — HTTP or direct import — reaches it. Routing the frontend through real HTTP calls to `backend/api.py` remains the architecturally intended design and a reasonable follow-up; it just isn't what's running today. This note is appended rather than silently editing the original text above, so both the original design intent and the actual as-built deviation are visible side by side.
